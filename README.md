...
每天 **12:07（北京时间）** 由定时任务自动跑一次（实测触发会迟到 4.5～5.7 小时，见下文「已知问题」）：从 GitHub Trending 发现正在升温的项目，用**规则**筛掉教程 / Awesome List / 文档 / 数据集，用 GitHub API 补全客观数据，按**可解释的固定公式**算热度分，做多样性选择，最多挑出 **10 个**真正值得关注的软件项目，让 AI 写一份简洁中文日报，存进仓库形成长期历史，并发一封 HTML 邮件。
...
> 但实测**触发时间显著晚于 cron 设定值**（设 04:07 UTC，实际 09:22 / 09:47 UTC），
> 也就是报告在**北京 17:20～17:50** 送达，而不是 12:07。
