...
> ⚠️ **事后更正（2026-09-13）**：本文第 8 节 Q9 中"`timezone: 'Asia/Shanghai'` 是官方支持的
> IANA 时区"的结论**错误**。官方文档 `on.schedule` 小节没有 `timezone` 字段，
> 写了不生效、cron 仍按 UTC 解释。正确配置是直接用 UTC：
> 12:07 北京 → `cron: '7 4 * * *'`（无 `timezone`）。
> 另外，本仓库的 `schedule` 触发器实测从未成功触发过（详见 README 的已知问题）。
